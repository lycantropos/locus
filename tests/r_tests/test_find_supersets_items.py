...
def test_base_boxes(tree: Tree[ScalarT]) -> None:
    assert all(
        (index, box) in tree.find_supersets_items(box)
...
def test_properties(tree_with_box: tuple[Tree[ScalarT], Box[ScalarT]]) -> None:
    ...
    assert all(is_subset_of(box, result_box) for _, result_box in result)
    assert all(item in result for item in items if is_subset_of(box, item[1]))
